...
\noindent \textbf{Perturbation data.} All prioritization runs against the aggregated supplementary tables of a
...
\noindent \textbf{Disease and program vocabularies.} The program \textbf{B} is fixed to Th1/Th2 polarization, with two term
...
\noindent \textbf{Literature and association sources.} Co-mention counts come from Europe PMC \cite{europepmc2015} \texttt{hitCount} queries (all
...
\noindent \textbf{Model cast.} Inside the workbench (Section 3.4) the author role is an Opus-class model (Claude Opus
...
\noindent \textbf{A disease-answer-free gene universe.} The candidate gene set \textbf{A} is defined from the perturbation
...
\noindent \textbf{Five deterministic signals.} For each (gene A, disease C) pair we compute: \texttt{ab}, the Europe PMC
...
\noindent \textbf{Gate and ranking objective.} Eligibility requires all of: \texttt{ab\ $\geq$\ ab\_gate} (a universe percentile,
...
\noindent \textbf{Order of operations.} The sweep builds \textbf{A}, prefetches the per-disease \texttt{bc}/\texttt{ac\_known} (12 calls
...
(gene, disease) pairs $\rightarrow$ 43 supported (positive disease hop with an admissible effect) $\rightarrow$ 30 clean full-chain nominations.
...
\item
  ...
  failed experiment into a ``we cannot say,'' rather than a false ``no effect.''
\item
...
effect hops; the disease hop it can only \emph{prioritize} as a genetic-association nomination. Two caveats follow from this separation, and belong in Methods, not only in Limitations. First, because \textbf{A}
...
real transcriptional effect is \emph{refuted} at hop 1). That out-of-funnel ledger is quantified in Section 4.2b.
...
\caption{\textbf{The Wayfinder architecture: construction separated from adjudication.} The generator
builds a disease-answer-free universe of knockdown-gated, program-significant genes and applies a
literature-novelty gate, yielding 22{,}039 eligible (gene, disease) pairs (\emph{construction filters},
top). A deterministic referee then evaluates each pair hop by hop and returns one verdict ---
\emph{supported}, \emph{refuted}, or \emph{untested} --- with a data receipt at every hop (\emph{referee
decisions}, bottom). Hops 0--2 (knockdown QC, effect, program) are experimental receipts the referee owns;
the disease hop is an association receipt whose near-total stringency is inherited from the substrate
(\S4.1b). The whole loop runs inside an agentic workbench (Claude Science) with an independent
language-critic self-audit; cross-vendor independence is supplied externally by a five-member replication
lab (\S4.6).}
...
\noindent \textbf{Operating an instrument that has no API.} Claude Science exposes no programmatic task-submission
...
raises on any live call, so unattended approval cannot silently alter the data receipts. This
is browser automation against a user interface with no stable public contract (Section 5.3); it is
...
\noindent \textbf{Provenance and self-audit.} Every run is captured in the workbench's own audit store, from which we
...
substrate's to make, not the critic's. We scope this claim precisely: the enforcement applies
...
\noindent \textbf{Three claims about liveness, kept distinct.} We separate what ran live from what was replayed, because
