...
must carry receipts and carefully scoped verdicts --- was applied to the machine that generated it.
...
the workbench, reproduced value-identically (the two outputs differ only by line-ending), verified by the workbench's own reviewer. An agentic analysis that
...
indicated a per-cell reduction rather than a shift in cell-type composition --- \emph{significant} in the
keratinocyte compartment (-0.51, paired \emph{p} 0.027) and in the same direction but \emph{not} significant in
the T/NK compartment (-0.57, \emph{p} 0.11), so we read it as a partial per-cell confirmation. Read together, these are more
...
There is a real ceiling on this. Expression tracks the direction of \emph{association}, not the
distinction between an effector and a brake; the voting-arm evidence was mixed (one significant arm, others
ambiguous or null); and the brake reading carries a specific mechanistic caveat --- NAB2's one described T-cell
role is as an EGR-family co-regulator (\S{}4.3), and EGR2 is itself STAT6-driven and \emph{pro}-type-2, so if
NAB2's Th2 effect is mediated through EGR2 the brake-versus-effector sign could invert, which is exactly why the
directional claim awaits a perturbation. Only a perturbation --- showing that \emph{restoring} NAB2 dampens the
Th2 program --- can convert this brake hypothesis into a directional therapeutic claim. That perturbation is the experiment the
...
We follow the LBD tradition of a thorough limitations section, and there is a great deal to cover.
...
LD-inherited from the adjacent 12q13 atopy locus. We ruled out the \emph{cis}-effect on STAT6's expression, which
...
label-shuffle control (\S{}4.1b) is the most important negative control in the paper, and its result cuts
against a credulous reading: the disease hop supports fewer than 1\% of arbitrary gene$\times$disease pairs, but so does a
...
platform --- not a durable interface. If the front-end changes, the driver must be re-fitted.
...
establish that the referee \emph{decides correctly at scale}. We have now taken the first measured step against
that gap --- a time-sliced held-out evaluation (\S{}4.7) that returns a null at its pre-registered primary
metric --- and broader external validation remains the programme of work the loop nominates concretely.

Four steps, in priority order, take these behavioural diagnostics toward a measured decision-quality claim; the
first we have now taken. \textbf{(1) A time-sliced held-out evaluation} \cite{yetisgen2009}, \emph{carried out
here} (\S{}4.7): freezing the literature at 2016 and measuring recovery of
gene$\rightarrow$program$\rightarrow$disease relationships established only afterward, it returns a \emph{null}
at the pre-registered primary (precision@20, two-way-clustered CI) --- the full pipeline does not outrank
literature-rarity or the disease-enrichment hop alone, though it leads at the very top of the list (precision@5)
and on mean average precision. The single fixed twelve-disease substrate makes the clustered interval wide and
the test low-powered; extending it across cutoffs and to a larger disease panel is the immediate follow-on, and
converts ``the machinery runs'' toward ``the machinery is right.'' \textbf{(2) An external
