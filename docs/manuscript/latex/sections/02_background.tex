...
ranked \cite{henry2017,henry2019}, and have applied the paradigm across biomedicine --- including our own use of an ABC pipeline to
...
novel simply because no one has studied it \cite{stoeger2018}. The result is a long ranked list that no laboratory can work
...
of which ever get followed up''}. Standard evaluations of LBD (time-slicing, link prediction \cite{yetisgen2009,moreau2023}) measure
whether the generator would have \emph{rediscovered} known links, not whether a specific proposed hypothesis is
supported by independent experimental data it has not seen \cite{henry2021}. We are precise about the scope of the answer this enables: prioritizing a
...
prioritization step: a way to put each machine-generated hypothesis to a data test and record a verdict ---
including a \emph{no}.
...
The prioritization step needs data that exists independently of the hypotheses being posed. We use a
genome-scale CRISPRi Perturb-seq \cite{dixit2016,adamson2016,replogle2022} resource in primary human CD4+ T cells \cite{zhu2025},
...
is not a universal oracle of biological truth; it is a bounded, independent evidence surface for
...
association signal for the fourth is what keeps the eventual claims within what the receipts support.
...
The generation, evidence integration, and provenance in this work were carried out inside an \emph{agentic} language-
...
data-grounded prioritization, and a self-audit of the result's language on one bench --- the capability the
rest of this paper puts to work.

\subsection*{2.4 Related approaches, and the gap Wayfinder occupies}\label{related-approaches}

Three lines of work bear on the triage problem, and Wayfinder is best understood by what it borrows from
each and what it withholds. \emph{Classical LBD ranking} --- from Arrowsmith \cite{smalheiser1998} through
PubMed-scale predication stores \cite{kilicoglu2012} to modern co-occurrence and graph systems
\cite{pyysalo2019} --- ranks candidate A--C links by properties of the \emph{literature}: co-mention
statistics, association measures, semantic-vector similarity, and knowledge-graph link-prediction scores
(reviewed in \cite{thilakaratne2019,gopalakrishnan2019}). These return a ranked list with a numeric
plausibility, but the score is a literature-derived statistic; such systems do not put a candidate to an
independent \emph{data} test, cannot abstain on data-quality grounds, and do not return a \emph{refuted}.
Wayfinder consumes an LBD-style candidate but replaces the literature-similarity score with a data
adjudication.

\emph{LLM hypothesis generation} now produces fluent, often-novel biomedical hypotheses, and a benchmarking
literature has grown around it. The recurring failure mode is over-confident hallucination: reliability-focused
work explicitly targets truthfulness \cite{truthhypo2025}, and, most pointedly for us, a recent contrastive
study shows that a language model's \emph{plausibility} judgements about cellular perturbations diverge from
held-out CRISPR ground truth \cite{plausibility2026} --- direct motivation for grounding the decision in a
deterministic experimental gate rather than in model judgement.

\emph{Agentic ``AI-scientist'' systems} are the closest neighbours, and we are careful not to strawman them.
Google's AI co-scientist is built around critique and ranking --- a tournament of simulated scientific debate
--- and reports wet-lab-validated hits \cite{gottweis2025}; FutureHouse's agents attach source citations to
claims and, in Robin, analyse real assay data inside the loop \cite{paperqa2024,robin2025}; SciAgents reasons
over an ontological knowledge graph with an explicit critic \cite{sciagents2025}; and other systems run their
own experiments end-to-end in chemistry and machine learning \cite{boiko2023,aiscientist2024}. These are
capable and, in several cases, experimentally validated. We are not aware, however, of a system that does the
specific combination Wayfinder targets: a \textbf{deterministic, non-LLM referee} that scores each hypothesis against a
\textbf{held, pre-existing} experimental substrate and returns a per-hop \emph{experimental} receipt --- an
odds ratio, a $p$-value, an effect size --- for every causal edge, with a quality-control-gated
\textbf{abstention} (a failed knockdown reported as \emph{untested}, not negative) and a \textbf{falsification}
as first-class verdicts. Their receipts, where present, are literature citations (PaperQA2) or the outputs of
model debate; and the systems that \emph{do} touch experimental data --- Robin, which analyses real assay
data, and the chemistry and machine-learning systems that run their own experiments --- generate that
evidence from \emph{new} experiments rather than adjudicating each hypothesis against a \emph{held,
pre-existing} substrate. They optimise, moreover, for producing \emph{plausible} hypotheses, whereas the
deliverable here is the confident, receipt-backed \emph{no}. Even approaches that benchmark language models against perturbation
readouts \cite{perturbqa2025} frame the task as prediction accuracy, not as a referee that abstains and
refutes with a receipt. The gap Wayfinder occupies is therefore narrow and specific: an LBD front-end paired
with a deterministic experimental back-end, reporting prioritization, abstention, and falsification --- each
with a receipt --- in a division of labour where the language model's visible agency is reserved for judgement
and provenance, never for computing the evidence.
