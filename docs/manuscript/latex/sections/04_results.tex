...
post-critic difference \emph{is} the self-audit. The same headless operation reproduces the held-out evaluation
of \S{}4.7 blind and to the same broad-null verdict, extending this self-audit from the finding to the
\emph{measurement} (Figure 6).
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{fig6_cs_reproduction}
\caption{\textbf{The held-out evaluation independently reproduced inside Claude Science (\S{}4.5), driven
headlessly and blind to the expected values.} \emph{(A)} precision@$k$ for the six rankers as computed by the
local harness (lines) and, overlaid, by Claude Science (open markers): the markers fall on the lines --- the two
computations agree at every point. \emph{(B)} the two co-primary contrasts, local (blue) versus Claude Science
(orange), each a point estimate and a gene-and-disease two-way-clustered bootstrap 95\% CI: the point estimates
are identical ($C_{\mathrm{broad}} = +0.20$, $C_{\mathrm{mech}} = -0.15$) and both intervals span zero, so the
workbench returns the same broad-null verdict. Interval widths differ slightly because the two runs draw from
independent random-number streams (NumPy in the workbench, the standard library locally). This is the self-audit
discipline of \S{}4.5 applied to the evaluation itself: the metric computation --- where the review pipeline
earlier caught a bootstrap-weighting error --- is confirmed by a second, blind implementation.}
\label{fig:csrepro}
\end{figure}
