...
baselines and returns a null at its pre-registered primary metric.
...
\noindent \textbf{Contributions.} Stated up front --- what we found, then how --- so a reader can judge
relevance before the details:
\begin{enumerate}
\tightlist
\item \textbf{A biological finding: NAB2 as a literature-novel Th1/Th2 \emph{polarization} regulator.} A targeted
  literature audit surfaced no papers linking NAB2 to Th1/Th2 polarization or to atopic eczema; the perturbation
  data support NAB2 as a polarization regulator with a receipt at every hop, distinct from its one described
  T-cell role as an EGR-family activation coregulator (its paralog NAB1 moves the program the opposite way;
  \S{}4.3). We nominate it toward atopic eczema, exclude the sharpest artifact --- a STAT6 \emph{cis}-effect ---
  at the expression level, and foreground the one confounder we cannot discharge (12q13 LD provenance) rather
  than claim it resolved.
\item \textbf{How we found it: a receipt-backed prioritization architecture.} LBD hypothesis generation paired
  with a \emph{deterministic, non-LLM} referee that adjudicates each gene$\rightarrow$program$\rightarrow$disease
  triple against a held Perturb-seq substrate and returns a per-hop receipt, with two first-class diagnostics
  ...
  judge --- is the methodological contribution, positioned against evaluated near-neighbours in \S{}2.4.
\item \textbf{A measurement of the method, not only a demonstration.} A pre-registered, time-sliced held-out
  evaluation (\S{}4.7) returns a \emph{null} at its primary metric: the pipeline does not rank future-established
  links above literature-rarity or the disease-enrichment hop alone, and the residual ranking signal traces to
  the substrate-inherited disease hop.
...
\end{enumerate}
\noindent \textbf{What we do not claim:} predictive correctness or superior ranking (measured --- null);
