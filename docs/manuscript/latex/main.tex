...
% --- title ---
\title{\vspace{-2em}\bfseries\color{sectioncolor}Receipt-backed, self-auditing literature-based discovery\\
  in an agentic workbench (Claude Science):\\
  prioritizing and stress-testing Perturb-seq hypotheses}
...
and ranking digit-for-digit, and the sensitivity panel value-identically (replicable in principle, subject to
...
resolved; an independent cross-model replication reproduced the finding. We then submit the method to a
measurement rather than asserting its value: a time-sliced held-out evaluation --- freezing the literature at
2016 and testing whether the pipeline ranks links established only afterward above literature-rarity and
single-axis baselines --- returns a \emph{null} at its pre-registered primary metric, with what ranking signal
there is carried by the substrate's disease-enrichment hop. The contribution is the
method and what it found: \textbf{receipt-backed prioritization of machine-generated hypotheses, with explicit
abstention and falsification diagnostics}, evaluated rather than asserted --- not a demonstration of
