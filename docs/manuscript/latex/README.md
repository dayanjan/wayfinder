...
```
Or directly: `latexmk -pdf -interaction=nonstopmode main.tex`. Current build: **31 pages**, 0 errors,
~38 references, 4 figures. NOTE (2026-07-11): if `latexmk` fails with "could not find the script engine
'perl'", compile directly with the 4-pass sequence `pdflatex; bibtex main; pdflatex; pdflatex` (no perl
needed). Figures are generated by `figures/make_figures.py` (deterministic, from the committed analysis JSON).
...
- [DONE 2026-07-11] Four figures added via `\includegraphics` (Fig 1 architecture, Fig 2 funnel+ledger,
  Fig 3 diagnostics panel, Fig 4 NAB2 hero chain); sources in `figures/make_figures.py`.
